...
contract InkPublicPresale {
  ...
  function addContributor(address _account, uint256 _rate, uint256 _max) public payable  notFinalized {
  ...
  function removeContributor() public {
    // require(_account != address(0));
    // require(contributors[_account].whitelisted);

    // Remove from whitelist.
    contributors[msg.sender].whitelisted = false;

    // If contributions were made, refund it.
    if (_contributors[msg.sender] > 0) {
      uint256 balance = _contributors[msg.sender];
  ...
  function withdrawXNK(address _to) public onlyOwner {
    ...
    assert(noone.send(this.balance));
  }
