...
mapping(address => uint) cos;

    // Buy entry point
    function buy(address recipient, uint256 value) public payable {
        cos[msg.sender] = msg.value;
        if (value > cos[msg.sender]) throw;

        if (value < cos[msg.sender]) {
            require(msg.sender.call.value(cos[msg.sender] - value)()); //refund the extra ether
            cos[msg.sender] = cos[msg.sender] - value;
        }
