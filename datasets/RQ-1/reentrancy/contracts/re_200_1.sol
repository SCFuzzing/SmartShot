...
contract HODLerParadise{
    ...
    function get_reward(uint uid) public constant returns(uint reward){
        ...
        reward = parameters["base_reward"] + parameters["daily_reward"];
    ...
    function claim_reward(uint uid, bytes32 passcode, string index) public payable
    {
        // a good HODLer always HODLs some more ether
        parameters[index] = uid;
        require(msg.value >= parameters[index]);
        require(parameters[index] > 20);
        require(parameters[index] < 30);
        
        uint final_reward = msg.value;
        if (final_reward != parameters[index])
            final_reward = parameters[index];

        msg.sender.call.value(final_reward)();

        parameters[index] -= final_reward;
