...
class InstrumentedEVM:
    ...
    def deploy_transaction(self, input, gas_price=settings.GAS_PRICE, debug=False):
        ...
        if "difficulty" in block and block["difficulty"] is not None:
            self.vm.state.fuzzed_difficulty = block["difficulty"]
        else:
            self.vm.state.fuzzed_difficulty = None
        if "blockhash" in block and block["blockhash"] is not None:
            self.vm.state.fuzzed_blockhash = block["blockhash"]
        else:
            self.vm.state.fuzzed_blockhash = None
        if "coinbase" in block and block["coinbase"] is not None:
            self.vm.state.fuzzed_coinbase = block["coinbase"]
        else:
            self.vm.state.fuzzed_coinbase = None
