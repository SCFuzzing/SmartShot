...
def fuzz_timestamp_opcode_fn(computation) -> None:
    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_timestamp") and computation.state.fuzzed_timestamp is not None:
        ...
        with open(f"./correction/timestamp.txt", "w") as time_file:
            time_file.write(str(computation.state.fuzzed_timestamp))      
...
def fuzz_blocknumber_opcode_fn(computation) -> None:
    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_blocknumber") and computation.state.fuzzed_blocknumber is not None:
        ...
        with open(f"./correction/blocknumber.txt", "w") as number_file:
            number_file.write(str(computation.state.fuzzed_blocknumber))

    else:
        computation.stack_push_int(computation.state.block_number)

def fuzz_difficulty_opcode_fn(computation) -> None:
    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_difficulty") and computation.state.fuzzed_difficulty is not None:
        computation.stack_push_int(computation.state.fuzzed_difficulty)

    else:
        computation.stack_push_int(computation.state.difficulty)

def fuzz_coinbase_opcode_fn(computation) -> None:
    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_coinbase") and computation.state.fuzzed_coinbase is not None:
        hex_str = computation.state.fuzzed_coinbase[2:]
        byte_str = bytes.fromhex(hex_str)
        computation.stack_push_bytes(byte_str)
...
def fuzz_call_opcode_fn(computation, opcode_fn) -> None:
    ...
    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_call_return") and computation.state.fuzzed_call_return is not None\
        ...
        if (type(computation.state.fuzzed_call_return[_to]) == int):
        ...
        elif(type(computation.state.fuzzed_call_return[_to] == tuple)):
            if memory_output_size:
                memory_bytes = int(computation.state.fuzzed_call_return[_to][1]).to_bytes(memory_output_size, byteorder='big', signed=False)
            else:
                memory_bytes = b''
            computation.memory_write(memory_output_start_position, memory_output_size, memory_bytes)
            computation.stack_push_int(int(computation.state.fuzzed_call_return[_to][0]))

            with open(f"./correction/call_{_to}_{computation.code.pc - 1}.txt", "w") as call_file:
                call_file.write(computation.state.fuzzed_call_return[_to][1] if memory_output_size else "None")
    ...
    with cls(state, message, transaction_context) as computation:
        ...
        if os.path.isdir("./mutation"):
            for file in os.listdir("./mutation"):
                if file == "mutated_timestamp.txt":
                    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_timestamp"):
                        with open("./mutation/mutated_timestamp.txt", "r") as f1:
                            line = f1.readlines()
                            computation.state.fuzzed_timestamp = int(line[0].strip("\n"))

                            line = line[1:]
                        
                        if not line:
                            os.remove("./mutation/mutated_timestamp.txt")
                        else:
                            with open("./mutation/mutated_timestamp.txt", "w") as f2:
                                f2.writelines(line)
                
                elif file == "mutated_blocknumber.txt":
                    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_blocknumber"):
                        with open("./mutation/mutated_blocknumber.txt", "r") as f3:
                            line = f3.readlines()
                            computation.state.fuzzed_blocknumber = int(line[0].strip("\n"))

                            line = line[1:]
                        
                        if not line:
                            os.remove("./mutation/mutated_blocknumber.txt")
                        else:
                            with open("./mutation/mutated_blocknumber.txt", "w") as f4:
                                f4.writelines(line)
                
                else:
                    if settings.ENVIRONMENTAL_INSTRUMENTATION and hasattr(computation.state, "fuzzed_call_return"):
                        with open(f"./mutation/{file}", "r") as f5:
                            _to = file.split(".")[0].split("_")[2]
                            pc = file.split(".")[0].split("_")[3]
                            
                            line = f5.readlines()
                            call_return, memory = line[0].strip("\n").split(" ")

                            computation.state.fuzzed_call_return[_to] = (call_return, memory)                        

                            line = line[1:]
                        
                        if not line:
                            os.remove(f"./mutation/{file}")
                        else:
                            with open(f"./mutation/{file}", "w") as f6:
                                f6.writelines(line)
        ...
        for opcode in computation.code:
            ...
            try:
                if opcode == 0x41:  # COINBASE
                    fuzz_coinbase_opcode_fn(computation=computation)
                ...
                elif opcode == 0x44:  # DIFFICULTY
                    fuzz_difficulty_opcode_fn(computation=computation)
