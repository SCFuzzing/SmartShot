    ...
    def run(self, ng):
        ...
        try:
            ...
            while g < ng or settings.GLOBAL_TIMEOUT:
                ...
                for a in self.analysis:
                    if g % a.interval == 0:
                        self.snapshots = a.register_step(g=g, population=self.population, engine=self)

                self.population.individuals = indvs[:size]
                g += 1
