...
class Individual():
    ...
    def decode(self):
        ...
        for i in range(len(self.chromosome)):
            ...
            if "difficulty" in self.chromosome[i] and self.chromosome[i]["difficulty"] is not None:
                block["difficulty"] = copy(self.chromosome[i]["difficulty"])
            if "blockhash" in self.chromosome[i] and self.chromosome[i]["blockhash"] is not None:
                block["blockhash"] = copy(self.chromosome[i]["blockhash"])
            if "coinbase" in self.chromosome[i] and self.chromosome[i]["coinbase"] is not None:
                block["coinbase"] = copy(self.chromosome[i]["coinbase"])
