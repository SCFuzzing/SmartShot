...
class Generator:
    def __init__(self, interface, bytecode, accounts, contract, constants, onchain_status):
        ...
        self.last_mutated_pool = {}

    def gaussian_noise_mutate(self, original_input):
        multipliers = [0.01, 0.02, 0.05, 0.1]
        multiplier_base = random.choice(multipliers)
        multiplier = abs(np.random.normal(0, multiplier_base / 3, 1)[0])
        
        return int(original_input * (1 + multiplier))
    ...
    def generate_random_individual(self):
        ...
        if "constructor" in self.interface and self.bytecode:
            ...
            if self.onchain_status:
                individual.append({
                    "account": self.get_random_account("constructor"),
                    "contract": self.bytecode,
                    "amount": self.get_random_amount("constructor"),
                    "arguments": arguments,
                    "blocknumber": self.onchain_status["block_number"],
                    "timestamp": self.onchain_status["timestamp"],
                    "gaslimit": self.onchain_status["gas_limit"],
                    "difficulty": self.onchain_status["difficulty"],
                    "blockhash": self.onchain_status["blockhash"],
                    "coinbase": self.onchain_status["coinbase"],
                    "returndatasize": dict()
                })
            else: 
                individual.append({
    ...
    def get_random_timestamp(self, function):
        if function in self.timestamp_pool:
            self.last_mutated_pool["timestamp"] = int(self.timestamp_pool[function].head_and_rotate())
            return self.last_mutated_pool["timestamp"]
        else:
            seed = random.random()
            if seed < 0.25 and self.constants:
                result = int(random.choice(self.constants))
                if 1404183684 < result < 2035335684:
                    self.last_mutated_pool["timestamp"] = result
                    return result
            elif seed < 0.5 and "timestamp" in self.last_mutated_pool:
                result = self.gaussian_noise_mutate(self.last_mutated_pool["timestamp"])
                if 1404183684 < result < 2035335684:
                    self.last_mutated_pool["timestamp"] = result
                    return result
            elif seed < 0.75 and "timestamp" in self.last_mutated_pool:
                past = random.choice([60, 3600, 86400, 86400*7, 86400*30, 86400*365])
                result = self.last_mutated_pool["timestamp"] + past
                if 1404183684 < result < 2035335684:
                    self.last_mutated_pool["timestamp"] = result
                    return result
                
        result = random.randint(1404183684, 2035335684)
        self.last_mutated_pool["timestamp"] = result
        return result
    ...
    def get_random_blocknumber(self, function):
        if function in self.blocknumber_pool:
            self.last_mutated_pool["blocknumber"] = int(self.blocknumber_pool[function].head_and_rotate())
            return self.last_mutated_pool["blocknumber"]
        else:
            seed = random.random()
            if seed < 0.25 and self.constants:
                result = int(random.choice(self.constants))
                if 0 < result < 100000000:
                    self.last_mutated_pool["blocknumber"] = result
                    return result
            elif seed < 0.5 and "blocknumber" in self.last_mutated_pool:
                result = self.gaussian_noise_mutate(self.last_mutated_pool["blocknumber"])
                if 0 < result < 100000000:
                    self.last_mutated_pool["blocknumber"] = result
                    return result
            elif seed < 0.75 and "blocknumber" in self.last_mutated_pool:
                past = random.choice([1, 10, 100, 1000, 10000, 100000, 1000000])
                result = self.last_mutated_pool["blocknumber"] + past
                if 0 < result < 100000000:
                    self.last_mutated_pool["blocknumber"] = result
                    return result
                
        result = random.randint(1000000, 30000000)
        self.last_mutated_pool["blocknumber"] = result
        return result
