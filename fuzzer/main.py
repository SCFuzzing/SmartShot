...
from utils.utils import initialize_logger, compile, get_interface_from_abi, get_pcs_and_jumpis, get_function_signature_mapping, get_onchain_status
...
class Fuzzer:
    ...
    def run(self):
        ...
        with open(self.args.source, "r") as file:
            source = file.read()
        
        pattern = r"(?<![\w.])(?:[1-9]\d*|0\d+)(?![\w.])"

        constants = re.findall(pattern, source)

        onchain_status = {}
        if self.args.onchain_address:
            color = "\u001b[34m"
            if self.args.block_number:
                logger.title(color+"Requesting onchain status WITH given block number")
            else:
                logger.title(color+"Requesting onchain status WITHOUT given block number")
            onchain_status = get_onchain_status(self.args.onchain_address, self.args.block_number, "eth", logger)
        elif self.args.onchain_address_bsc:
            if self.args.block_number:
                logger.title(color+"Requesting onchain status WITH given block number")
            else:
                logger.title(color+"Requesting onchain status WITHOUT given block number")
            onchain_status = get_onchain_status(self.args.onchain_address, self.args.block_number, "bsc", logger)
        ...
        engine.analysis.append(ExecutionTraceAnalyzer(self.env, onchain_status))
...
def main():
    ...
    if not os.path.isdir("./correction"):
        os.mkdir("correction")
...
def launch_argument_parser():
    ...
    # Onchain analysis
    parser.add_argument("-oc", "--onchain-address", help="The deployed contract address on Ethereum for onchain analysis", action="store",
                        type=str)
    parser.add_argument("-oc-b", "--onchain-address-bsc", help="The deployed contract address on BNB Smart Chain for onchain analysis", action="store",
                        type=str)
    parser.add_argument("-bn", "--block-number", help="The blocknumber of the block to deploy onchain states", action="store",
                        type=str)
