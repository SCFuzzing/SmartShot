                ...
                if tainted_record and tainted_record.stack and tainted_record.stack[-1]:
                    ...
                    if "calldataload" in index or "callvalue" in index:
                        ...
                        for _argument_index in [int(a.split("_")[-1]) for a in index.split() if a.startswith("calldataload_"+str(transaction_index)+"_")]:
                            if _argument_index >= len(individual.generator.interface[_function_hash]):
                                if individual.generator.interface[_function_hash][0] == "string":
                                    _is_string = True
                            elif individual.generator.interface[_function_hash][_argument_index] == "string":
                                _is_string = True
                        if not _is_string:
