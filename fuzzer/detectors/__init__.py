...
class DetectorExecutor:
    ...
    def input_correction(self, mfe, solution):
        if mfe.last_used_var:
            color = DetectorExecutor.get_color_for_severity("Correction")
            self.logger.title(color+"-----------------------------------------------------")
            self.logger.title(color+"Correction for Inputs:")
            self.logger.title(color+"-----------------------------------------------------")
            if not mfe.taint_counter:        
                for var in mfe.last_used_var:
                    if var.startswith("calldataload"):
                        input_transaction_index = str(int(var.split("_")[1]) + 1)
                        input_index = str(int(var.split("_")[2]) + 1)
                        input_ = str(mfe.last_used_var[var])
                        self.logger.title(color+"Input No."+input_index+" of Transaction No."+input_transaction_index+" Should be "+input_)
                
            else:
                for index, transaction in enumerate(solution):
                    if transaction.get("transaction"):
                        if transaction["transaction"].get("data"):
                            for var in mfe.last_used_var:
                                if var.startswith("calldataload"):
                                    input_transaction_index = str(int(var.split("_")[1]) + 1)
                                    input_index = str(int(var.split("_")[2]) + 1)
                                    if input_transaction_index == str(index + 1) and input_index == "1":
                                        try:
                                            input_ = int("0x"+transaction["transaction"]["data"][10:].lstrip("0"), 16)
                                        except:
                                            input_ = 0
                                        self.logger.title(color+"Input No."+input_index+" of Transaction No."+input_transaction_index+" Should be "+str(input_))
            self.logger.title(color+"-----------------------------------------------------")                

    def block_correction(self, mfe):
        for file in os.listdir("./correction"):
            if file.startswith("timestamp"):
                with open(f"./correction/{file}", "r") as time_file:
                    timestamp = time_file.readline().strip()
        
                color = DetectorExecutor.get_color_for_severity("Correction")
                self.logger.title(color+"-----------------------------------------------------")
                self.logger.title(color+"Correction for Timestamp:")
                self.logger.title(color+"-----------------------------------------------------")
                self.logger.title(color+"Timestamp of the Block Should be "+timestamp)
                self.logger.title(color+"-----------------------------------------------------")

            if file.startswith("blocknumber"):
                with open(f"./correction/{file}", "r") as number_file:
                    blocknumber = number_file.readline().strip()

                color = DetectorExecutor.get_color_for_severity("Correction")
                self.logger.title(color+"-----------------------------------------------------")
                self.logger.title(color+"Correction for Blocknumber:")
                self.logger.title(color+"-----------------------------------------------------")
                self.logger.title(color+"Blocknumber of the Block Should be "+blocknumber)
                self.logger.title(color+"-----------------------------------------------------")

    def call_correction(self, mfe):
        for _ in os.listdir("./correction"):
            if _.startswith("call"):
                color = DetectorExecutor.get_color_for_severity("Correction")
                self.logger.title(color+"-----------------------------------------------------")
                self.logger.title(color+"Correction for Call Returns:")
                self.logger.title(color+"-----------------------------------------------------")
                for file in os.listdir("./correction"):
                    if file.startswith("call"):
                        with open(f"./correction/{file}", "r") as call_file:
                            file = file.split(".")[0]
                            call_symbol, address, pc = file.split("_")
                            memory = call_file.readline().strip()
                        self.logger.title(color+"Call Return of "+address+" at "+pc+" Should be "+memory)
                self.logger.title(color+"-----------------------------------------------------")        
                break
    ...
    def run_detectors(self, previous_instruction, current_instruction, errors, tainted_record, individual, mfe, previous_branch, transaction_index):
        ...
        if pc and DetectorExecutor.add_error(errors, pc, "Arbitrary Memory Access", individual, mfe, self.arbitrary_memory_access_detector, self.source_map):
            ...
            self.input_correction(mfe, individual.solution)
            self.block_correction(mfe)
            self.call_correction(mfe)
